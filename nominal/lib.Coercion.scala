...
object Coercion extends UniverseConstruction with util.Traverse {
  ...
  def goodieImpl[Actual, Expected]
    (c: Context)
    (arg: c.Tree)
    (tagT: c.Tree)
    (implicit
          ...
          (source, target) match {
            ...
            case (
              ...
              val maybeFunctor: Tree =
                if (
                  isScalaSubtype(c)(srcTpe, treeToType(c)(getRecordOrVariant(c)))
                ){
              ...
              maybeFunctor match {
                ...
                case functor =>
                  ...
                  for {
                    ...
                    newDfs = mkF { x => q"$functor($x).map(..$fs)" }
                  }
                  yield (an, newDfs +: dfs)
              }
  ...
  def isRecordOfVariant(c: Context)(lhs0: c.Type, rhs0: c.Type): Boolean = {
    ...
    val lhs = applyConstructorToNothing(c)(lhs0)

    val r0  = rhs0.dealias
    val rf  = r0.typeConstructor.etaExpand

    val rhs = rf.resultType.substituteTypes(
      rf.typeParams,
      r0.typeArgs.map { arg => applyConstructorToNothing(c)(arg) }
    )

    isScalaSubtype(c)(lhs, rhs)
  }
